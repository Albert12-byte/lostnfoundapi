...
class PrivateApiTests(TestCase):
    ...
    def test_retrieve_claims_admin_only(self):
        ...
        res = self.client.get(CLAIMS_URL)
        claims = [claim2, claim1]  # Assuming claims are ordered by ID in descending order
    ...
    def test_claim_list_limited_to_user(self):
        ...
        self.assertEqual(sorted(res.data, key=lambda x: x['id']), sorted(serializer.data, key=lambda x: x['id']))
    ...
    def test_match_claim_to_item_by_description(self):
        ...
        item1 = create_item(user=self.user, description='A black Samsung A22 with cracked screen')
        item2 = create_item(self.user,description= 'A white Iphone 13 with no issues')
        claim = create_claim(user = self.user, description='Black Samsung A22 cracked screen found')
